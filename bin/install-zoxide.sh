...
install() {
  ...
  *)
  ...
  tempdir=$(mktemp -d)
  release_url=$(curl -s https://api.github.com/repos/ajeetdsouza/zoxide/releases/latest | grep "browser_download_url" | cut -d '"' -f 4 | grep "$_target")
  warning "Detected release: $(basename "$release_url")"
  success "Downloading zoxide..."

  ## Downloading the binaries
  pushd $tempdir >/dev/null || exit 1
  ensure curl -LJO "$release_url"
  ensure tar zxf "$(basename "$release_url")"
  ensure mv zoxide "$_install_path/zoxide"
